...
class SnapTarget extends _core().Target {
  ...
  getDefaultStagePackages() {
    // libxss1, libasound2 - was "error while loading shared libraries: libXss.so.1" on Xubuntu 16.04
    // noinspection SpellCheckingInspection
    const result = ["libnspr4", "libnss3", "libxss1", "libappindicator3-1", "libsecret-1-0"]; // const result = []
  ...
  createDescriptor(arch) {
    ...
    return (0, _bluebirdLst().coroutine)(function* () {
      if (semver().lt(_this.packager.config.electronVersion || "5.0.3", "2.0.0-beta.1")) {
      ...
      (0, _builderUtil().deepAssign)(snap, {
        ...
        parts: {
          app: {
            "stage-packages": _this.replaceDefault(options.stagePackages, _this.getDefaultStagePackages())
          }
        }
      });
  ...
  build(appOutDir, arch) {
    ...
    return (0, _bluebirdLst().coroutine)(function* () {
      ...
      const stageDir = yield (0, _targetUtil().createStageDirPath)(_this2, packager, arch); // snapcraft.yaml inside a snap directory

      const snapMetaDir = path.join(stageDir, _this2.isUseTemplateApp ? "meta" : "snap");
      const args = ["snap", "--app", appOutDir, "--stage", stageDir, "--arch", (0, _builderUtil().toLinuxArchString)(arch, true), "--output", _this2.isUseTemplateApp ? artifactPath : "out.snap", "--executable", _this2.packager.executableName];
      ...
      if (semver().gte(_this2.packager.config.electronVersion || "5.0.3", "5.0.0") && !isBrowserSandboxAllowed(snap)) {
      ...
      if (_this2.isUseTemplateApp) {
        args.push("--template-url", semver().gte(_this2.packager.config.electronVersion || "5.0.3", "4.0.0") ? "electron4" : "electron2");
      }
      ...
      if (!_this2.isUseTemplateApp) {
        ...
        yield (0, _fsExtraP().rename)(path.join(stageDir, "out.snap"), artifactPath);
      }
